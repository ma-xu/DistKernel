...
import math
__all__ = ['combine8_resnet18', 'combine8_resnet34', 'combine8_resnet50', 'combine8_resnet101',
           'combine8_resnet152']
...
class AssConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=1, dilation=1, groups=1,bias=False):
        ...
        self.register_buffer('alpha', torch.ones([1,out_channels,1,1]))

        self.fc = nn.Sequential(
            nn.Conv2d(out_channels,out_channels//16,1,bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(out_channels//16,4*out_channels, 1, bias=False)
        )
    ...
    def forward(self, input):
        ...
        alpha = self.fc(self.alpha).reshape([1,4,-1,1,1])
...
def combine8_resnet18(pretrained=False, **kwargs):
...
def combine8_resnet34(pretrained=False, **kwargs):
...
def combine8_resnet50(pretrained=False, **kwargs):
...
def combine8_resnet101(pretrained=False, **kwargs):
...
def combine8_resnet152(pretrained=False, **kwargs):
...
def demo():
    ...
    for i in range(1):
        net = combine8_resnet18(num_classes=1000)
...
def demo2():
    st = time.perf_counter()
    for i in range(1):
        net = combine8_resnet50(num_classes=1000).cuda()
