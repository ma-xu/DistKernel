...
import math
__all__ = ['combine7_resnet18', 'combine7_resnet34', 'combine7_resnet50', 'combine7_resnet101',
           'combine7_resnet152']
...
class AssConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=1, dilation=1, groups=1,bias=False):
        ...
        self.register_buffer('alpha', torch.ones([1,4,1,1]))
    ...
    def forward(self, input):
        ...
        alpha = self.fc(self.alpha).unsqueeze(dim=-1)
...
def combine7_resnet18(pretrained=False, **kwargs):
...
def combine7_resnet34(pretrained=False, **kwargs):
...
def combine7_resnet50(pretrained=False, **kwargs):
...
def combine7_resnet101(pretrained=False, **kwargs):
...
def combine7_resnet152(pretrained=False, **kwargs):
...
def demo():
    ...
    for i in range(1):
        net = combine7_resnet18(num_classes=1000)
...
def demo2():
    st = time.perf_counter()
    for i in range(1):
        net = combine7_resnet50(num_classes=1000).cuda()
