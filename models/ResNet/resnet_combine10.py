...
import math
__all__ = ['combine10_resnet18', 'combine10_resnet34', 'combine10_resnet50', 'combine10_resnet101',
           'combine10_resnet152']
...
class AssConv(nn.Module):
    ...
    def forward(self, input):
        ori_out = self.ori_conv(input).unsqueeze(dim=1)
        second_out =self.second_conv(input).unsqueeze(dim=1)
        dilate_out = self.dilate_conv(input).unsqueeze(dim=1)
        group_out = self.group_conv(input).unsqueeze(dim=1)

        all_out = torch.cat([ori_out,second_out,dilate_out,group_out],dim=1) # N*4*C_out*W*H

        ori_temp_out = self.ori_conv(self.temp).unsqueeze(dim=1) # 1*1*C_out*7*7
        second_temp_out = self.second_conv(self.temp).unsqueeze(dim=1) # 1*1*C_out*7*7
        dilate_temp_out = self.dilate_conv(self.temp).unsqueeze(dim=1) # 1*1*C_out*7*7
        group_temp_out = self.group_conv(self.temp).unsqueeze(dim=1) # 1*1*C_out*7*7
...
def combine10_resnet18(pretrained=False, **kwargs):
...
def combine10_resnet34(pretrained=False, **kwargs):
...
def combine10_resnet50(pretrained=False, **kwargs):
...
def combine10_resnet101(pretrained=False, **kwargs):
...
def combine10_resnet152(pretrained=False, **kwargs):
...
def demo():
    ...
    for i in range(1):
        net = combine10_resnet18(num_classes=1000)
...
def demo2():
    st = time.perf_counter()
    for i in range(1):
        net = combine10_resnet50(num_classes=1000).cuda()
