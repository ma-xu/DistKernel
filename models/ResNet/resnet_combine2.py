...
import math
__all__ = ['combine2_resnet18', 'combine2_resnet34', 'combine2_resnet50', 'combine2_resnet101',
           'combine2_resnet152']
...
class AssConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=1, dilation=1, groups=1,bias=False):
        ...
        self.group16_conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, groups=16, bias=bias)
        ...
        self.group16_bn = nn.BatchNorm2d(out_channels)


    def forward(self, input):
        return self.ori_bn(self.ori_conv(input))+self.group4_bn(self.group4_conv(input))\
               +self.group16_bn(self.group16_conv(input))+self.dilate_bn(self.dilate_conv(input))
...
def combine2_resnet18(pretrained=False, **kwargs):
...
def combine2_resnet34(pretrained=False, **kwargs):
...
def combine2_resnet50(pretrained=False, **kwargs):
...
def combine2_resnet101(pretrained=False, **kwargs):
...
def combine2_resnet152(pretrained=False, **kwargs):
...
def demo():
    ...
    for i in range(1):
        net = combine2_resnet18(num_classes=1000)
...
def demo2():
    st = time.perf_counter()
    for i in range(1):
        net = combine2_resnet50(num_classes=1000).cuda()
