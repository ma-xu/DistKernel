...
import math
__all__ = ['validate6_resnet18', 'validate6_resnet34', 'validate6_resnet50', 'validate6_resnet101',
           'validate6_resnet152']
...
class AssConv(nn.Module):
    ...
    def forward(self, input):
        ...
        all_buffer = torch.cat([buffer1.unsqueeze(dim=1),buffer2.unsqueeze(dim=1),
                                buffer3.unsqueeze(dim=1),buffer4.unsqueeze(dim=1)],dim=1).unsqueeze(dim=-1)

        all_buffer = self.fc(all_buffer).unsqueeze(dim=-1)
...
def validate6_resnet18(pretrained=False, **kwargs):
...
def validate6_resnet34(pretrained=False, **kwargs):
...
def validate6_resnet50(pretrained=False, **kwargs):
...
def validate6_resnet101(pretrained=False, **kwargs):
...
def validate6_resnet152(pretrained=False, **kwargs):
...
def demo():
    ...
    for i in range(1):
        net = validate6_resnet18(num_classes=1000)
...
def demo2():
    st = time.perf_counter()
    for i in range(1):
        net = validate6_resnet50(num_classes=1000).cuda()
