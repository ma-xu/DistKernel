...
import math
__all__ = ['dist4_resnet18', 'dist4_resnet34', 'dist4_resnet50', 'dist4_resnet101',
           'dist4_resnet152']
...
class DPConv(nn.Module):
    def __init__(self, in_planes, out_planes, k=3, stride=1):
        ...
        self.distribution_bias = Parameter(torch.zeros(out_planes, in_planes, 1, 1)
                                           *F.adaptive_avg_pool2d(self._init_distribution(), 1))
        self.register_buffer('init_var', torch.tensor(2/(self.out_planes * self.k * self.k)))
        self.distribution_scale = Parameter(torch.ones(1)*
                                            math.sqrt(self.init_var / (self._init_distribution().var()))
                                            )
    ...
    def forward(self, input):
        ...
        for i in range(50):
            ...
            param = -(param - self.distribution_bias)
            param = self.distribution_scale * param
...
def dist4_resnet18(pretrained=False, **kwargs):
...
def dist4_resnet34(pretrained=False, **kwargs):
...
def dist4_resnet50(pretrained=False, **kwargs):
...
def dist4_resnet101(pretrained=False, **kwargs):
...
def dist4_resnet152(pretrained=False, **kwargs):
...
def demo():
    st = time.perf_counter()
    for i in range(1):
        net = dist4_resnet50(num_classes=1000)
