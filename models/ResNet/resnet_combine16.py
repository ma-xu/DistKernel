...
import math
__all__ = ['combine16_resnet18', 'combine16_resnet34', 'combine16_resnet50', 'combine16_resnet101',
           'combine16_resnet152']
...
class AssConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=1,bias=False):
        ...
        self.key = nn.Sequential(
            nn.Conv2d(out_channels,4*self.groups,1,groups=self.groups,bias=False),
            nn.ReLU(inplace=True)
        )
        self.query = nn.Sequential(
            nn.Conv2d(self.groups, 4 * self.groups, 1, groups=self.groups, bias=False),
            nn.ReLU(inplace=True)
        )
        self.transform = nn.Sequential(
            nn.Conv2d(self.groups, out_channels, 1, groups=self.groups, bias=False),
            nn.Sigmoid()
        )
    ...
    def forward(self, input):
        ...
        gap =self.gap(all_out)
        query = (gap.view(b,self.groups,c//self.groups,1,1)).mean(dim=2)
        query = self.query(query).view(b,4,self.groups,1,1)
        query = query.softmax(dim=1)
        key = self.key(gap).view(b,4,self.groups,1,1)
        transform = (key*query).sum(dim=1)
        transform = self.transform(transform)

        return all_out*transform
...
def combine16_resnet18(pretrained=False, **kwargs):
...
def combine16_resnet34(pretrained=False, **kwargs):
...
def combine16_resnet50(pretrained=False, **kwargs):
...
def combine16_resnet101(pretrained=False, **kwargs):
...
def combine16_resnet152(pretrained=False, **kwargs):
...
def demo():
    ...
    for i in range(1):
        net = combine16_resnet18(num_classes=1000)
...
def demo2():
    st = time.perf_counter()
    for i in range(1):
        net = combine16_resnet50(num_classes=1000).cuda()
