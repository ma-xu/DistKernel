...
import math
__all__ = ['dist9_resnet18', 'dist9_resnet34', 'dist9_resnet50', 'dist9_resnet101',
           'dist9_resnet152']
...
class DPConv(nn.Module):
    def __init__(self, in_planes, out_planes, k=3, stride=1):
        ...
        self.distribution_scale = Parameter(self._get_init_scale()*torch.ones(self.out_planes,self.in_planes,1,1))
        self.distribution_bias = Parameter(torch.zeros(self.out_planes,self.in_planes,1,1))
    ...
    def _get_init_scale(self):
        ...
        y = -(1.0 / (std * math.sqrt(2 * math.pi))) \
            * torch.exp(-((self.mask * self.distribution_zoom) ** 2) / (2 * (std ** 2)))
        y = y.permute(2, 3, 0, 1)
        scale = math.sqrt(kaiming_var/y.var())
        return scale
...
def dist9_resnet18(pretrained=False, **kwargs):
...
def dist9_resnet34(pretrained=False, **kwargs):
...
def dist9_resnet50(pretrained=False, **kwargs):
...
def dist9_resnet101(pretrained=False, **kwargs):
...
def dist9_resnet152(pretrained=False, **kwargs):
...
def demo():
    ...
    for i in range(1):
        net = dist9_resnet50(num_classes=1000)
...
def demo2():
    st = time.perf_counter()
    for i in range(50):
        net = dist9_resnet50(num_classes=1000).cuda()
