...
import math
__all__ = ['ablation41_resnet18', 'ablation41_resnet34', 'ablation41_resnet50', 'ablation41_resnet101',
           'ablation41_resnet152']

# PD-A: standard + AC + group
...
class AssConv(nn.Module):
    ...
    def forward(self, input):
        return self.ori_bn(self.ori_conv(input))+self.ac_convbn(input)+self.group_bn(self.group_conv(input))
...
def ablation41_resnet18(pretrained=False, **kwargs):
...
def ablation41_resnet34(pretrained=False, **kwargs):
...
def ablation41_resnet50(pretrained=False, **kwargs):
...
def ablation41_resnet101(pretrained=False, **kwargs):
...
def ablation41_resnet152(pretrained=False, **kwargs):
...
def demo():
    ...
    for i in range(1):
        net = ablation41_resnet18(num_classes=1000)
...
def demo2():
    # st = time.perf_counter()
    for i in range(1):
        net = ablation41_resnet50(num_classes=1000).cuda()
