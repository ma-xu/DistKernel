...
import math
__all__ = ['new3_six_resnet18', 'new3_six_resnet34', 'new3_six_resnet50', 'new3_six_resnet101',
           'new3_six_resnet152']
...
class AssConv(nn.Module):
    ...
    def forward(self, input):
        ...
        ac_2_out = self.ac_convbn(input).unsqueeze(dim=1)
        group_2_out = self.group_bn_2(self.group_conv_2(input)).unsqueeze(dim=1)

        all_out = torch.cat([ori_out, new_out, ac_out, group_out,ac_2_out,group_2_out], dim=1)
...
def new3_six_resnet18(pretrained=False, **kwargs):
...
def new3_six_resnet34(pretrained=False, **kwargs):
...
def new3_six_resnet50(pretrained=False, **kwargs):
...
def new3_six_resnet101(pretrained=False, **kwargs):
...
def new3_six_resnet152(pretrained=False, **kwargs):
...
def demo():
    ...
    for i in range(1):
        net = new3_six_resnet18(num_classes=1000)
...
def demo2():
    # st = time.perf_counter()
    for i in range(1):
        net = new3_six_resnet50(num_classes=1000).cuda()
